...
def calculate_activation(sum_total):
    ...
    """
    # Ensure sum_total is a Tensor
    sum_total_tensor = torch.tensor(sum_total) if not isinstance(sum_total, torch.Tensor) else sum_total
    
    # Create a tensor with the same type as sum_total_tensor filled with 0.0
    zero_tensor = torch.zeros_like(sum_total_tensor)
    
    return torch.heaviside(sum_total_tensor, zero_tensor)
